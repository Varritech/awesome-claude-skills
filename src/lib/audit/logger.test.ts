...
describe('logAuditEvent', () => {
  ...
  it('includes metadata when provided', async () => {
    ...
    vi.mocked(adminDb.collection).mockReturnValue({ doc: vi.fn().mockReturnValue({ set: mockSet }) } as any);
  ...
  it('does not throw when Firestore write fails (fire and forget)', async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    vi.mocked(adminDb.collection).mockReturnValue({ doc: vi.fn().mockReturnValue({ set: vi.fn().mockRejectedValue(new Error('Firestore down')) }) } as any);
