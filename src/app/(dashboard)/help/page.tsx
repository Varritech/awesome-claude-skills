...
function FaqItem({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <Card className="cursor-pointer" onClick={() => setOpen((v) => !v)}>
      ...
      {open && (
        <p className="text-[12px] text-white/35 mt-2.5 leading-relaxed">{answer}</p>
      )}
    </Card>
  );
}
  ...
  return (
    <>
      ...
      <div className="mt-8 mb-6">
        <p className="text-[13px] font-bold text-white/25 uppercase tracking-wide mb-3">
          ...
          {[
            {
              ...
              a: "Go to Settings > Connected Inboxes and click + Add Inbox. We support Gmail, Outlook, and custom SMTP. Most setups take under 2 minutes.",
            },
            {
              q: "How does inbox warmup work?",
              a: "ConvergeFlow gradually increases your daily send volume over 14 days — starting at 5 emails/day and adding 3 per day until you reach your limit. This builds sender reputation before your campaigns go live.",
            },
            {
              q: "What DNS records do I need to add?",
              a: "You need three records: an SPF TXT record to authorise your sending server, a DKIM TXT record for cryptographic signing, and a DMARC TXT record to set your policy. ConvergeFlow shows you the exact values to copy.",
            },
            {
              q: "How do I import leads?",
              a: "Go to Customers and click Import CSV. Your file needs at least one of: email, firstName, or lastName. Optional columns (company, title, industry, location) improve AI personalisation. Duplicate emails are skipped automatically.",
            },
            {
              q: "What happens when someone replies?",
              a: "Replies appear in the Replies inbox, classified automatically as interested, not interested, out of office, or booked. Sequences auto-stop for that lead. You can reply manually or let the AI draft a response.",
            },
            {
              q: "Can I customise the AI writing style?",
              a: "Yes. Go to Email Styles to choose from four personas (Closer, Neighbor, Expert, Helper) or build a custom style. You can also adjust tone and add your own context in the persona builder.",
            },
            {
              q: "How does lead scoring work?",
              a: "Each lead gets an engagement score (0–100) based on email interactions: +10 per open, +25 per reply, +50 if booked, -10 if bounced. Scores update automatically after each event.",
            },
            {
              q: "Can I cancel my subscription anytime?",
              a: "Yes — go to Settings > Plan & Billing and click Cancel Plan. Access continues until the end of your billing period. No cancellation fees.",
            },
          ].map(({ q, a }, i) => (
